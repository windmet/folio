...
if (!projectId || projectId.startsWith('-')) {
  console.error('Usage: npm run editorial:export-copy -- <project-slug>');
  process.exit(1);
}
...
const uiCopy = [
  ...
  ['project-archive.overview', 'heading', '一条线看懂这场活动', 'src/components/project/ProjectArchiveShell.astro'],
  ['project-archive.overview', 'kicker', 'FIVE-MINUTE ORIENTATION', 'src/components/project/ProjectArchiveShell.astro'],
  ['project-archive.overview', 'description', '这场近五小时直播最有趣的地方，是很多笑点会隔上一两个小时突然回来。最典型的一条从俄罗斯章鱼烧开始：有人先看见绿色、悄悄放回，赛马又把所有人的注意力带走，直到约 80 分钟后才由完全不知情的人踩雷。', 'src/components/project/ProjectArchiveShell.astro'],
  ...
  ['player-context-rail', 'node', '节点', 'src/components/project/PlayerContextRail.astro'],
  ['player-context-rail', 'clue', '线索', 'src/components/project/PlayerContextRail.astro'],
  ['player-context-rail', 'source', '原链', 'src/components/project/PlayerContextRail.astro'],
  ['player-context-rail', 'related', 'RELATED STORYLINES', 'src/components/project/PlayerContextRail.astro'],
...
const markdownPath = path.join(outputDirectory, `${projectId}-reader-copy-review.md`);
const manifestPath = path.join(outputDirectory, `${projectId}-reader-copy-manifest.json`);
