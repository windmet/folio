...
> 状态：RC 0.12 PLANNED（2026-08-09；RC 0.11 REVIEW BRANCH ACCEPTANCE COMPLETE）
...
> **CURRENT CHECKPOINT**
> RC 0.10 已完成本地验收，RC 0.11 已完成 R9 与 review-branch handoff；其 acceptance 文档保持历史合同，不因新一轮视觉问题回写。真实 1366×768 People 页面随后发现组件级越界与 Player 权重问题，现独立进入 RC 0.12 Visual Polish & Source Navigation Pass。当前实施权威为 `docs/editorial/komatsu36-rc12-visual-polish-source-navigation-plan.md`；RC 0.11 历史流程、产品合同、Payload 合同与发布交接仍分别见 `komatsu36-rc11-closeout-runbook.md`、`komatsu36-archive-navigation-pass.md`、`komatsu36-static-payload-pass.md`、`komatsu36-rc11-release-readiness-handoff.md`。Release Gate 继续 CLOSED。

## 当前实施状态（2026-08-09）

> **RC 0.12 状态覆盖**：RC 0.11 required 功能批次已全部完成，R6 为 `NO ADDITIONAL MOBILE NAV FOR V1`，R8 的 F/G 为 `DEFERRED BY DEFAULT` / `NOT NEEDED FOR V1`。RC 0.12 现作为独立视觉收尾阶段 PLANNED，下一批为 RC12-A；下方较早的 RC 0.11 历史段落只保留为演进记录。Release Gate 仍独立关闭。
...
> **当前结论（RC 0.12 planning）**：下方 1–15 项是历史实施顺序。RC 0.11 的最终状态仍以 `docs/editorial/komatsu36-rc11-closeout-runbook.md` 与 `docs/editorial/komatsu36-rc11-release-readiness-handoff.md` 为准；下一实现批次改为 RC12-A，不再把“只等待 Release Gate”误作当前待办。Release Gate 继续等待独立授权。
...
16. **RC 0.12（planned）**：先做 Adaptive Player、可展开文本与 People / Cast layout normalization，再人工停点；Media pictogram 为 P1，Source-scoped Timeline 默认延后 v1.1。不得回写 RC 0.11 acceptance 合同，完整逐批流程见 `docs/editorial/komatsu36-rc12-visual-polish-source-navigation-plan.md`。
...
| Cast / People 信息架构 | **RC 0.11 移动合同完成；RC 0.12 新 P0**：1366×768 下 document overflow 为 0、Cast table 在 808px 内容列内无溢出，但 `.people-grid` client width 808px / scroll width 1112px，证明三列 PersonCard 存在组件级越界。RC12-C 必须按内容列做 responsive projection，并同时保留 Cast medium fallback |
