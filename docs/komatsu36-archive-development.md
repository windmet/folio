...
> 状态：RC12-T1 REQUIRED — READY FOR IMPLEMENTATION（2026-08-10；RC 0.11 REVIEW BRANCH ACCEPTANCE COMPLETE）
...
> **CURRENT CHECKPOINT**
> RC 0.10 已完成本地验收，RC 0.11 已完成 R9 与 review-branch handoff；其 acceptance 文档保持历史合同，不因新一轮视觉问题回写。RC12-A2/C2/D2/B2 与 F2 已完成工程与 Browser 验证，RC12-E E1–E5 已完成工程与本地真实 Browser 验证。2026-08-10 新产品复核新增必修 RC12-T1，只处理 Timeline Navigator 的 shell-width geometry、极窄 segment 投影和 hover／keyboard-focus 完整标签；审阅红框／蓝框只是几何标注，不是产品配色方向。当前下一批唯一实施权威为 `docs/editorial/komatsu36-rc12-t1-timeline-navigator-polish-runbook.md`，总状态仍由 product correction runbook 与 PRODUCT-CHECKPOINT 汇总；E5、F2 文档保留对应证据。Release Gate 继续 CLOSED。

## 当前实施状态（2026-08-10）

> **RC 0.12 状态覆盖**：RC 0.11 required 功能批次已全部完成，R6 为 `NO ADDITIONAL MOBILE NAV FOR V1`，R8 的 F/G 为 `DEFERRED BY DEFAULT` / `NOT NEEDED FOR V1`。RC12-A2、C2、D2、B2 与 F2 已通过 SOURCE/BROWSER 证据；RC12-E E1–E5 已完成三 scope、native local clock、source-local Event projection、URL/history、390px、Enter／Space、无障碍语义和播放器边界 Browser 回归。当前新必修为 RC12-T1；T1 完成工程／Browser QA 与人工停点前，不能关闭 RC 0.12 产品接受。Y1/Y2 YouTube handoff 只登记为后续候选，尚未授权。真实媒体与 Release Gate 继续独立关闭。
...
18. **RC12-T1（下一批）**：产品复核要求把 YT 8-Act Navigator 做成 Project shell 级全局章节带，保留真实 duration ratio，让极窄 segment 只显示 Axx，并为 A01–A08 提供 hover/focus 完整标签。不得修改 Event、Track、scope、URL、current Act 或 Player playback；详细合同见 `docs/editorial/komatsu36-rc12-t1-timeline-navigator-polish-runbook.md`。
...
| Timeline Navigator polish | **RC12-T1 REQUIRED / NOT IMPLEMENTED**：Expanded/Docked 使用 Project shell 可用宽度；保留 duration ratio；窄 segment 只显示 Axx；A01–A08 hover/focus 显示完整标签。红／蓝框仅为 QA 几何标注，不进入产品色板。入口见 `docs/editorial/komatsu36-rc12-t1-timeline-navigator-polish-runbook.md` |
