...
> **CURRENT CHECKPOINT**
> RC 0.10 已完成本地验收，RC 0.11 已完成 R9 与 review-branch handoff；其 acceptance 文档保持历史合同，不因新一轮视觉问题回写。真实 1366×768 People 页面随后发现组件级越界与 Player 权重问题，独立开启 RC 0.12 Visual Polish & Source Navigation Pass。RC12-A2/C2/D2/B2 与 F2 已完成工程与 Browser 验证；用户 2026-08-10 本地简要复核反馈“基本能接受”，随后按继续指令完成 RC12-E E1–E5 工程与本地真实 Browser 验证。当前实施权威为 `docs/editorial/komatsu36-rc12-product-correction-runbook.md`，E1–E3 基础证据见 `docs/qa/komatsu36-rc12/e/README.md`，E5 完整交接见 `docs/qa/komatsu36-rc12/e/E5-HANDOFF.md`，F2 交接见 `docs/editorial/komatsu36-rc12-f2-final-regression-handoff.md`；第一版 Plan 与旧 handoff 仅保存历史证据。RC 0.11 历史流程、产品合同、Payload 合同与发布交接仍分别见 `komatsu36-rc11-closeout-runbook.md`、`komatsu36-archive-navigation-pass.md`、`komatsu36-static-payload-pass.md`、`komatsu36-rc11-release-readiness-handoff.md`。Release Gate 继续 CLOSED。

## 当前实施状态（2026-08-10）

> **RC 0.12 状态覆盖**：RC 0.11 required 功能批次已全部完成，R6 为 `NO ADDITIONAL MOBILE NAV FOR V1`，R8 的 F/G 为 `DEFERRED BY DEFAULT` / `NOT NEEDED FOR V1`。RC12-A2、C2、D2、B2 与 F2 已通过 SOURCE/BROWSER 证据；RC12-E E1–E5 已完成三 scope、native local clock、source-local Event projection、URL/history、390px、Enter／Space、无障碍语义和播放器边界 Browser 回归。用户复核基本接受并允许继续收口，但不等同于真实媒体通过或 Release Gate 授权。产品接受仍独立停点。F2 交接见 `docs/editorial/komatsu36-rc12-f2-final-regression-handoff.md`，E 基础证据见 `docs/qa/komatsu36-rc12/e/README.md`，E5 交接见 `docs/qa/komatsu36-rc12/e/E5-HANDOFF.md`。下方较早的 RC 0.11 历史段落只保留为演进记录。Release Gate 仍独立关闭。
...
17. **RC 0.12-E（当前）**：Source-scoped Timeline 的 E1–E5 已完成工程与本地真实 Browser 验证，覆盖三 source-local scope、Event projection、URL/history、Enter／Space、focus restoration、390px、无障碍语义、single player mount 和 console/overflow；完整交接见 `docs/qa/komatsu36-rc12/e/E5-HANDOFF.md`。当前仅保留产品人工停点、真实媒体、生产和 Release Gate 边界，不把 Browser 证据升级为 `PRODUCT-ACCEPTED`。
...
| Source-scoped Timeline | **RC12-E E1–E5 已完成源码与本地真实 Browser QA**：YT 8 Act／104 Event 保留；SP1 8、SP2 12 各自使用 native local clock 与 source-local Event projection；scope URL、YT／SP1 Event 深链、Back／Forward、SP2 直链刷新、external no-iframe、single mount、focus indicator、Enter／Space、无障碍语义和 390px overflow 已复测。产品停点仍待继续。证据见 `docs/qa/komatsu36-rc12/e/README.md` 与 `docs/qa/komatsu36-rc12/e/E5-HANDOFF.md` |
