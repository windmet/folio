...
21. **RC12 Final Interaction & Hierarchy Cleanup（P0 + P1 收尾）**：最新审阅明确 Person/Thread overlay 必须 XOR 互斥；Person → 相关事件线改为同层替换，Back 通过 history 恢复 Person。YT Player 删除重复 Act Context，非 Timeline 仅保留 Timeline CTA，SP1/SP2 改为“关联上下文”。随后已实施 P1-C `.archive-label-zh` 中文 taxonomy token 与 P1-D 默认收起的 Media Sources native disclosure，并在 390px／240px 压力视口完成 Browser QA；Source Event Index、source selector 与 `?track=` 合同保持不变。当前只等待产品复核。合同见 `docs/editorial/komatsu36-rc12-final-interaction-hierarchy-cleanup.md`。
