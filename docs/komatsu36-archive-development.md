...
> **CURRENT CHECKPOINT**
> RC 0.10 A0 / A / B / C / D / E 已完成本地验收；RC 0.11 UX11-A / B 已提交为 `effa314`，UX11-C Desktop Player Context Rail 已在 review branch `1162ba2` 完成；R1 opaque Player + Lead Person 与 R2 UX11-P0 payload audit 已完成。下一批是 UX11-P1，随后严格执行 P2，再做 UX11-D、UX11-E 决策与 UX11-H。当前流程权威为 `docs/editorial/komatsu36-rc11-closeout-runbook.md`，产品合同见 `docs/editorial/komatsu36-archive-navigation-pass.md`，Payload 合同见 `docs/editorial/komatsu36-static-payload-pass.md`。本文后续 Phase / 历史 Next Step 保留为设计演进记录；冲突时以上述三份 RC 0.11 文档为准。
...
15. **RC 0.11（实施中）**：UX11-A / B 已提交为 `effa314`；UX11-C Desktop Player Context Rail、R1 opaque Player / `00 HOST / BIRTHDAY` 与 R2 UX11-P0 audit 已完成。下一批按 UX11-P1/P2 动态化 124 个 Source Event Index buttons、把 158 项 Search 索引迁移到 Astro build-time static JSON；记录新基线后才进入左内容列的 proportional 8-Act Timeline Navigator。Mobile 先裁决唯一合同；Playhead 默认延后，Quick / Detail 保持条件项；最终执行 UX11-H。完整逐批流程见 `docs/editorial/komatsu36-rc11-closeout-runbook.md`。
...
| Initial HTML payload | **RC 0.11 P0 已完成**：固定 audit 输出 R1 后 354,388 raw bytes、Gzip 69,334、Brotli 35,690，仍通过 350 KiB hard gate；Search section 73,355 bytes、Source Event Index 24,973 bytes；下一步独立移除工具型重复 DOM，不裁减 Rail |
...
上述工程、媒体与 RC 0.10 编辑 Blocker 已完成，UX11-C 与 R1 视觉层级修正已在 review branch 完成。RC 0.11 当前执行 UX11-P Static Payload Pass；它只抽离 Search / Source Index 的工具型重复投影，保留 Timeline / Thread / Person 的静态 reader content，不改变资料、Rail、媒体能力或 URL schema。之后继续 UX11-D、UX11-E 决策与 UX11-H；F/G 按 Runbook 裁决。全部完成后仍只剩明确的 Release Gate。
