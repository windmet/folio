...
21. **RC12 Final Interaction & Hierarchy Cleanup（P0 + P1 收尾）**：最新审阅明确 Person/Thread overlay 必须 XOR 互斥；Person → 相关事件线改为同层替换，Back 通过 history 恢复 Person。YT Player 删除重复 Act Context，非 Timeline 仅保留 Timeline CTA，SP1/SP2 改为“关联上下文”。随后已实施 P1-D 默认收起的 Media Sources native disclosure，并在 390px／240px 压力视口完成 Browser QA；Source Event Index、source selector 与 `?track=` 合同保持不变。旧的 `.archive-label-zh` 方案已由最终视觉批次统一替换为 `.archive-taxonomy` 小号衬线语义 token，结构级 kicker 恢复英文 mono。
22. **RC12 Final Visual Closeout Batch（本批）**：Thread 节点统一进入 `navigateThreadEventToTimeline()`，多 Thread Event 使用单一 native chooser CTA，Hero 三项统计从 track/thread 数据派生并在移动端隐藏。实现、门禁与 Browser 矩阵见 `docs/editorial/komatsu36-rc12-final-visual-closeout-batch.md`；在真实路由 QA 完成前不得标记产品接受。
...
| Expandable text | **RC12-B2 Source title 机制保留；Act / Timeline title 已由 text-overflow-and-link-hierarchy pass 取代**：Source title 继续按真实 overflow 提供展开／收起；Act 正文标题统一两行，Timeline navigator 使用短标签与完整 tooltip，不再显示标题展开按钮。新合同见 `docs/editorial/text-overflow-and-link-hierarchy-pass.md`；原 RC12-B2 证据仍作为历史记录保留。 |
