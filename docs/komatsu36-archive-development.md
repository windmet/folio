...
> 状态：RC 0.12 PRODUCT REVIEW RECORDED — A2/C2/D2/B2 CORRECTION REQUIRED（2026-08-09；RC 0.11 REVIEW BRANCH ACCEPTANCE COMPLETE）
...
> **CURRENT CHECKPOINT**
> RC 0.10 已完成本地验收，RC 0.11 已完成 R9 与 review-branch handoff；其 acceptance 文档保持历史合同，不因新一轮视觉问题回写。真实 1366×768 People 页面随后发现组件级越界与 Player 权重问题，独立开启 RC 0.12 Visual Polish & Source Navigation Pass。RC12-A2/C2/D2/B2 与 F2 已完成工程与 Browser 验证；用户 2026-08-10 本地简要复核反馈“基本能接受”，随后按继续指令启动 RC12-E E1–E3，已完成三来源 native-clock Timeline scope 与 Browser 回归。当前实施权威为 `docs/editorial/komatsu36-rc12-product-correction-runbook.md`，E1–E3 证据见 `docs/qa/komatsu36-rc12/e/README.md`，F2 交接见 `docs/editorial/komatsu36-rc12-f2-final-regression-handoff.md`；第一版 Plan 与旧 handoff 仅保存历史证据。RC 0.11 历史流程、产品合同、Payload 合同与发布交接仍分别见 `komatsu36-rc11-closeout-runbook.md`、`komatsu36-archive-navigation-pass.md`、`komatsu36-static-payload-pass.md`、`komatsu36-rc11-release-readiness-handoff.md`。Release Gate 继续 CLOSED。

## 当前实施状态（2026-08-09）

> **RC 0.12 状态覆盖**：RC 0.11 required 功能批次已全部完成，R6 为 `NO ADDITIONAL MOBILE NAV FOR V1`，R8 的 F/G 为 `DEFERRED BY DEFAULT` / `NOT NEEDED FOR V1`。RC12-A2、C2、D2、B2 与 F2 已通过 SOURCE/BROWSER 证据；RC12-E E1–E3 已完成三 scope、native local clock、source-local Event projection、URL/history 和 390px Browser 回归。用户复核基本接受并允许继续收口，但不等同于真实媒体通过或 Release Gate 授权。E4/E5 仍待完成，产品接受仍独立停点。F2 交接见 `docs/editorial/komatsu36-rc12-f2-final-regression-handoff.md`，E 证据见 `docs/qa/komatsu36-rc12/e/README.md`。下方较早的 RC 0.11 历史段落只保留为演进记录。Release Gate 仍独立关闭。
...
> **当前结论（RC 0.12 correction in progress）**：下方 1–16 项是历史实施顺序。RC 0.11 的最终状态仍以 `docs/editorial/komatsu36-rc11-closeout-runbook.md` 与 `docs/editorial/komatsu36-rc11-release-readiness-handoff.md` 为准；RC12-A/B/C/D 第一版虽已通过源码和 Browser QA，但产品复核已要求 A2 Docked Player、C2 People/Cast 桌面投影、D2 Source 层级／图标和 B2 文本覆盖返工。唯一实施入口为 `docs/editorial/komatsu36-rc12-product-correction-runbook.md`。不得把“LOCAL QA COMPLETE”或“只等待 Release Gate”误作当前状态。Release Gate 继续等待独立授权。
...
16. **RC 0.12 第一版（A/B/C/D + local F QA complete；产品未通过）**：Adaptive Player、Target / Context 按需展开、People / Cast layout normalization、Media platform pictograms 已完成第一版工程验证；A/B/C 证据见 `docs/qa/komatsu36-rc12/abc/README.md`，D 证据见 `docs/qa/komatsu36-rc12/d/README.md`，旧交接草案见 `docs/editorial/komatsu36-rc12-release-readiness-handoff.md`。2026-08-09 产品复核已将其退回 A2/C2/D2/B2，不得再写“只待人工接受”。Source-scoped Timeline 保留为显式 P1 backlog。不得回写 RC 0.11 acceptance 合同；第一版计划见 `docs/editorial/komatsu36-rc12-visual-polish-source-navigation-plan.md`，后续唯一入口见 `docs/editorial/komatsu36-rc12-product-correction-runbook.md`。
...
Media Pass、RC 0.9 Structural Editorial Audit、RC 0.10 Reader & Entity Editorial Pass 与 RC 0.11 均已通过对应本地验收。RC 0.12 只处理新发现的视觉／交互收尾，并继续让 Release Gate 独立等待明确授权：
...
| Source-scoped Timeline | **RC12-E E1–E3 已完成源码与 Browser QA**：YT 8 Act／104 Event 保留；SP1 8、SP2 12 各自使用 native local clock 与 source-local Event projection；scope URL、Event 深链、Back／Forward、external no-iframe 和 390px overflow 已复测。E4/E5 与产品停点仍待继续。证据见 `docs/qa/komatsu36-rc12/e/README.md` |
| Initial HTML payload | **RC12-E 已复测**：raw `291,352`、Gzip `50,580`、Brotli `31,989`，350 KiB hard gate 余量 `67,048`；初始 Source / Search buttons 仍为 0，Search 项、124 controller Event records 和三 track projection 通过；本轮未以减重名义删除 Rail 或 reader-facing 数据 |
