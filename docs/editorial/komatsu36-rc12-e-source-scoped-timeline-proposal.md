...
本文把 RC12-E 需要实施的最小合同、数据边界和验证批次写清楚。用户此前的“继续吧”被作为启动下一批的范围确认；E1–E5 已完成第一版实现、静态合同和本地真实 Browser 验证，产品停点和真实媒体边界仍未关闭。本提案不能把现有 Source Event Index、来源图标或 Source card 误报成 Source-scoped Timeline。
...
E1–E5 已在当前 review branch 完成工程与本地真实 Browser 验证；完整矩阵见 `docs/qa/komatsu36-rc12/e/E5-HANDOFF.md`。产品停点、真实媒体、生产部署和 Release Gate 仍未关闭。
