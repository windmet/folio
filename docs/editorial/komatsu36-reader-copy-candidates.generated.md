# Komatsu36 Reader Copy Candidates (Generated)

内容快照：2026-08-11-reader-copy-reviewed-final

> 本文件完全由脚本生成，禁止在这里填写人工决定。命中词只表示需要人工判断，不代表事实错误。所有决定只写入 `komatsu36-reader-copy-decisions.yml`。

## 使用合同

- 范围：126 个 Event 与 16 条 Thread 的当前发布文案。
- Event 只要存在 `qualification` 就进入候选，作为内部不确定性的人工复核队列；当前组件不会直接显示 qualification。其余项目按工程词、证据词和编辑元语言命中。
- 不确定性不改变读者理解时，保留内部 `qualification`，读者侧不显示。
- 不确定性改变人物归属、事件结果或叙事因果时，才新增自然语言 `readerNote`。
- 本候选集不处理 Transcript、Evidence、Chat 原文，也不增加 Event。
- `copyAction` 与 `readerNoteAction` 是两个独立维度；正文需要改写不代表必须增加 reader note。

## 快照

- Event candidates: 46
- Thread candidates: 10
- Recorded decisions: 56
- 扫描词组：`evidence-language`、`editorial-meta-language`、`technical-language`；完整词表以生成脚本为准。

## Event candidates

### RCOPY-001 · yt-000100-stream-start

- Track / Time: YT ≈00:01:00
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-000100-stream-start.json`

**CURRENT TITLE**

主直播正式开始

**CURRENT SUMMARY**

近五小时主直播从这里开始，开场先介绍场地、技术和活动安排。

**CURRENT QUALIFICATION**

canonical 只锁定为 00:01 左右，页面将其作为导航锚点，不主张逐秒入点。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把可用时间轴、canonical 和导航锚点留在内部资格说明，读者只需要知道节目从这里开始。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 近五小时主直播从这里开始，开场先介绍场地、技术和活动安排。
- Reader note: —

### RCOPY-002 · yt-002044-first-space-return

- Track / Time: YT ≈00:20:44
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-002044-first-space-return.json`

**CURRENT TITLE**

小松从第一次 Space 返回主直播

**CURRENT SUMMARY**

小松第一次从 Space 回到主直播；后面还会继续在两边来回跑。

**CURRENT QUALIFICATION**

返回点按 canonical 约 00:20:44 定位。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 用交替推进替代跨平台往返与现场轨，保留跨平台结构但移除工程术语。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 小松第一次从 Space 回到主直播；后面还会继续在两边来回跑。
- Reader note: —

### RCOPY-003 · yt-002541-second-space-departure

- Track / Time: YT ≈00:25:41
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-002541-second-space-departure.json`

**CURRENT TITLE**

小松再次离席，主直播与 Space 同时继续

**CURRENT SUMMARY**

小松第二次转去 Space；那边还在继续聊，主直播这边也没有停。

**CURRENT QUALIFICATION**

主直播离席点约为 00:25:41；这里只标记并发关系，不换算 Space 本地时钟。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把双轨、并发标注与本地时钟换算留在内部资格说明，直接描述两个现场同时发生。

**APPROVED REPLACEMENT**

- New title: 小松再次离席，主直播与 Space 同时继续
- New summary: 小松第二次转去 Space；那边还在继续聊，主直播这边也没有停。
- Reader note: —

### RCOPY-004 · yt-010711-first-guests-arrive

- Track / Time: YT ≈01:07:11
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-010711-first-guests-arrive.json`

**CURRENT TITLE**

狩野、滨、汐谷与井上等第一批来宾到场

**CURRENT SUMMARY**

第一批《俺知》相关来宾进入主区，前面一个多小时的开场准备终于真正变成多人打ち上げ。

**CURRENT QUALIFICATION**

多人进入与自我介绍存在重叠，页面只发布批次级到场锚点。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案已能说明第一批来宾进入主区，限定只存在于内部资格说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-005 · yt-011036-terashima-arrives

- Track / Time: YT ≈01:10:36
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-011036-terashima-arrives.json`

**CURRENT TITLE**

寺岛惇太加入第一批来宾

**CURRENT SUMMARY**

寺島到场后，昼夜两位武市演员都在现场。后面的三张生日券和舞台复盘，也都有了本人参与。

**CURRENT QUALIFICATION**

到场点按 canonical 约 01:10:36 定位。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案自然说明昼夜两位武市演员的关系。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-006 · yt-011311-host-returns

- Track / Time: YT ≈01:13:11
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-011311-host-returns.json`

**CURRENT TITLE**

小松回到已聚齐第一批来宾的主区

**CURRENT SUMMARY**

长时间 Space 往返暂时告一段落，小松回到已经聚齐第一批来宾的主区，大家开始正式庆生、看礼物和吃东西。

**CURRENT QUALIFICATION**

返回点按 canonical 约 01:13:11 定位。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案只保留回到主区及后续庆生段落，内部时间限定不向读者暴露。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-007 · yt-012117-cigar-chocolate

- Track / Time: YT ≈01:21:17
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-012117-cigar-chocolate.json`

**CURRENT TITLE**

狩野送来雪茄形巧克力

**CURRENT SUMMARY**

狩野的礼物被确认是雪茄形巧克力，小松叼着它制造出“36 岁吸烟出道”的视觉玩笑。

**CURRENT QUALIFICATION**

礼物品类与现场玩笑已锁定；品牌和具体商品仍需包装画面复核，页面不作商品级断言。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 品牌和商品级不确定性不改变雪茄形巧克力这一事件理解。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-008 · yt-013800-thirty-four-yen

- Track / Time: YT ≈01:38:00
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-013800-thirty-four-yen.json`

**CURRENT TITLE**

被放弃的 36J 最后只值 34 円

**CURRENT SUMMARY**

寺島揭晓剩余的 J 券只有 34 円，现场以“原来 1J 就是 1 円”收掉这个安全选项的玩笑。

**CURRENT QUALIFICATION**

34 円是当场揭晓的笑点；页面不把 J 当作需要建立固定汇率的正式单位。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案准确保留节目内的 34 円玩笑，没有把 J 扩写成正式单位。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-009 · yt-013840-great-payback

- Track / Time: YT ≈01:38:40
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-013840-great-payback.json`

**CURRENT TITLE**

八千万梦想把生日会说成“大还元祭”

**CURRENT SUMMARY**

Big Dream 的夸张奖金想象一路滑向点高价酒；小松随后解释频道收入缺少回馈场景，现场便把生日会称作“大還元祭”。

**CURRENT QUALIFICATION**

页面保留节目内的夸张玩笑与回馈理念，不把假设金额或消费对话当作实际交易记录。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 假设奖金和消费对话的限定保留在内部资格说明，读者文案已经是自然摘要。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-010 · yt-014822-race-interrupts

- Track / Time: YT ≈01:48:22
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-014822-race-interrupts.json`

**CURRENT TITLE**

大井赛马开跑，全场注意力转移

**CURRENT SUMMARY**

赛马开跑与章鱼烧被识破的录音直接相邻，制造了它被遗忘的条件；但当事人没有逐字确认“因为赛马才忘掉”。

**CURRENT QUALIFICATION**

赛马与遗忘具有强时序关联，但因果措辞保持限定。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案已经明确赛马与遗忘的关系是时序关联而非已证实因果。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-011 · yt-015525-second-guests-arrive

- Track / Time: YT ≈01:55:25
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-015525-second-guests-arrive.json`

**CURRENT TITLE**

光富、堀金与佐藤等第二批来宾进入主区

**CURRENT SUMMARY**

观剧与 18TRIP 朋友组加入，重叠的自我介绍把现场扩展成第二张人物桌，也开启新一轮礼物与跨平台电话。

**CURRENT QUALIFICATION**

canonical 将约 01:55:25 起锁为批次到场；多人自我介绍重叠，不做逐句 speaker 切分。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 批次到场与自我介绍重叠属于内部定位限制，读者摘要本身无需改写。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-012 · yt-015910-tea-chatgpt

- Track / Time: YT 01:59:10
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-015910-tea-chatgpt.json`

**CURRENT TITLE**

堀金用 ChatGPT 调查后送来红茶

**CURRENT SUMMARY**

堀金蒼平先用 ChatGPT 调查小松的喜好，再选择 MARIAGE FRÈRES 红茶；紧接着另一只 GODIVA 袋又被说明里面并不是 GODIVA。

**CURRENT QUALIFICATION**

红茶品牌与事件已锁定；具体茶款、GODIVA 袋内物品及后续“撞礼物”细节仍需画面或短窗复核。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 品牌和礼物细节的复核限制不改变主要事件，读者文案保持当前摘要。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-013 · yt-020048-baystars-towel

- Track / Time: YT ≈02:00:48
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-020048-baystars-towel.json`

**CURRENT TITLE**

光富把 18TRIP 与横滨爱好做成 BayStars 毛巾

**CURRENT SUMMARY**

光富崇雄吸取去年 bucket hat 可能被埋没的经验，把 18TRIP 的横滨背景和自己对 BayStars 的爱好合并成毛巾礼物，又引出“厕所毛巾一军”的用途争论。

**CURRENT QUALIFICATION**

BayStars 毛巾的品类与礼物动机已解决；具体商品、球员和款式仍属于画面复核项。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 礼物品类与动机已经足够支撑事件，具体商品限制不影响读者理解。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-014 · yt-020445-candle-fire

- Track / Time: YT ≈02:04:45
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-020445-candle-fire.json`

**CURRENT TITLE**

熊谷的蜡烛先要求小松收拾房间

**CURRENT SUMMARY**

熊谷送来蜡烛，现场却判断小松必须先收拾房间才能点燃，否则现在使用只会变成火灾或大骚动。

**CURRENT QUALIFICATION**

蜡烛品类与现场玩笑已锁定；品牌音形不稳定，页面不根据近音商品名猜补。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 蜡烛与收拾房间的现场玩笑已是完整读者摘要，品牌不确定性留在内部说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-015 · yt-020555-uchida-call-announced

- Track / Time: YT ≈02:05:55
- Status: `verified`
- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/events/yt-020555-uchida-call-announced.json`

**CURRENT TITLE**

主桌说明内田会通过电话加入

**CURRENT SUMMARY**

小松收到个人联络后离开主区，转去处理 LINE 电话和 Space；主桌这边则继续自己的聊天。

**CURRENT QUALIFICATION**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 将另一轨道替换为读者可理解的电话另一边，保留分流事实。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 小松收到个人联络后离开主区，转去处理 LINE 电话和 Space；主桌这边则继续自己的聊天。
- Reader note: —

### RCOPY-016 · yt-021540-milk-wordplay

- Track / Time: YT ≈02:15:40
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-021540-milk-wordplay.json`

**CURRENT TITLE**

M!LK 点歌被继续改成饮酒口令

**CURRENT SUMMARY**

留守组把 M!LK 的歌继续改造成喝酒口令；汐谷唱歌时狩野又在旁边吃 pizza，让副节目进一步失控。

**CURRENT QUALIFICATION**

歌曲与改词事件可成立；“滅／メッツ”等短词音形仍低置信度，因此页面不发布逐字口令。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 不发布逐字口令不妨碍读者理解歌曲被改成饮酒玩笑。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-017 · yt-022240-public-drinking-pace

- Track / Time: YT ≈02:22:40
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-022240-public-drinking-pace.json`

**CURRENT TITLE**

“普通饮酒会节奏”面对约 2900 名观众

**CURRENT SUMMARY**

大家自己都觉得现在已经完全是普通饮酒会的节奏，随后又被提醒：此刻还有大约 2900 人在看。

**CURRENT QUALIFICATION**

约 2900 是节目当时口头提到的即时规模，不作为最终播放量或精确统计发布。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 2900 作为现场当时的口头规模已在摘要中限定为观看现场，不冒充最终统计。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-018 · yt-022700-makoto-theme

- Track / Time: YT ≈02:27:00
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-022700-makoto-theme.json`

**CURRENT TITLE**

光富从昼场前后两次“誠”看见人物答案

**CURRENT SUMMARY**

光富选择沖田与以蔵谈“誠”的前段，因为它会在故事高潮被以蔵自己的“誠”反向照亮，让符号变成人物选择。

**CURRENT QUALIFICATION**

主题前后呼应已达到事件级；播放中的舞台台词与现场评论重叠，页面不冻结录像逐字原文。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有摘要自然说明誠如何从符号变成人物选择。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-019 · yt-025037-kano-toilet

- Track / Time: YT ≈02:50:37
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-025037-kano-toilet.json`

**CURRENT TITLE**

狩野在俳句附近提出去厕所

**CURRENT SUMMARY**

狩野的 dad joke 与おじさん化吐槽持续发展，并在俳句段附近暂时离席。

**CURRENT QUALIFICATION**

当前只锁定约 02:50:37 的事件级位置，不发布逐字引用。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 事件级离席摘要足够，未把未核实的逐字桥段写入正文。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-020 · yt-025449-kano-haiku

- Track / Time: YT 02:54:49
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-025449-kano-haiku.json`

**CURRENT TITLE**

狩野把“眼睛飞出来了”写进庆生俳句

**CURRENT SUMMARY**

狩野把祝贺、眼睛飞出去和“めでたい”硬凑成一首临场俳句，现场就这么继续玩了下去。

**CURRENT QUALIFICATION**

事件和核心构造已锁定；页面只作摘要，不发布完整逐字俳句。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有摘要保留俳句构造和喜剧线，不需要完整逐字稿。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-021 · yt-025829-hair-grab

- Track / Time: YT ≈02:58:29
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-025829-hair-grab.json`

**CURRENT TITLE**

髪掴み从安全机制变成“歯白っ”

**CURRENT SUMMARY**

寺島一度忘记髪掴み需要小松主动配合，排练时真的抓疼而收到正式投诉；本番改用更近脸距维持压迫感，却被小松刚处理得很白的前牙分散注意。

**CURRENT QUALIFICATION**

髪、受け身机制、正式投诉与“歯白っ”闭环已锁定；本番替代动作的单一动词仍不作法证级逐字发布。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 动作机制、投诉与歯白っ的闭环已经清楚，法证级逐字限制不属于读者说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-022 · yt-030202-yano-arrives

- Track / Time: YT ≈03:02:02
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-030202-yano-arrives.json`

**CURRENT TITLE**

矢野奖吾作为后到来宾加入

**CURRENT SUMMARY**

矢野在俳句段落中途到场，随后带出太阳镜礼物、生日俳句与 Bingo 未中奖的连续小线。

**CURRENT QUALIFICATION**

到场点按 canonical 约 03:02:02 定位。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 后到来宾带出的连续小线已清楚，canonical 只存在于内部定位。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-023 · yt-031258-blade-accident-recount

- Track / Time: YT ≈03:12:58
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-031258-blade-accident-recount.json`

**CURRENT TITLE**

复盘昼场断刀事故

**CURRENT SUMMARY**

现场说明濱侧刀折断、矢野侧只是削损，碎片飞向舞台袖且无人受伤。

**CURRENT QUALIFICATION**

当前锚点是事故复盘入口；细节来自人工裁决稿，不把舞台发生时刻伪装成视频时刻。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 标题与摘要描述的是复盘入口，事故原始发生时刻的区别留在内部资格说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-024 · yt-031300-ryoma-line-callback

- Track / Time: YT ≈03:13:00
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-031300-ryoma-line-callback.json`

**CURRENT TITLE**

下一句恰是“刀で斬り合う時代は終わりじゃ”

**CURRENT SUMMARY**

真实断刀之后，下一句偏偏正好是坂本龍馬的「刀で斬り合う時代は終わりじゃ」。自然到不少观众一度以为这本来就是演出。

**CURRENT QUALIFICATION**

视频坐标表示复盘段而非舞台事故的原始发生时间。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 同步感和观众误以为是演出的笑点已经完整。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-025 · yt-031615-book-improv

- Track / Time: YT ≈03:16:15
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-031615-book-improv.json`

**CURRENT TITLE**

剧中争“本”意外卷入演员手里的真实台本

**CURRENT SUMMARY**

汐谷选择夜场武市爆发的“本”场景；稽古即兴中，剧中争夺的书与演员实际拿着的朗读台本重叠，狩野顺势把意外吸收到角色反应里。

**CURRENT QUALIFICATION**

“本／台本”重叠与角色即兴已锁定；具体抓取动作和多个回忆版本不合并成单一逐字原句。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 本与台本的重叠及即兴吸收已足够说明事件，不合并逐字版本。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-026 · yt-032139-kano-back-pain

- Track / Time: YT ≈03:21:39
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-032139-kano-back-pain.json`

**CURRENT TITLE**

狩野在本番四天前闪到腰，却没有让观众察觉

**CURRENT SUMMARY**

三人共同选择夜武市死亡场面，复盘才揭晓狩野约四天前ぎっくり腰；倒地时漏出的声音并非台词，但初看完全融进濒死芝居。

**CURRENT QUALIFICATION**

腰伤时序、非台词声音与观众未察觉均已锁定；不倒因为果写成“腰痛让芝居更真实”。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 文案明确区分腰伤时序与芝居效果，没有倒因为果。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-027 · yt-032828-seiten-opening

- Track / Time: YT 03:28:28
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-032828-seiten-opening.json`

**CURRENT TITLE**

清典 Pick 了整段 Opening

**CURRENT SUMMARY**

清典挑的不是某个单独招式，而是从朗读导入、名乘杀阵、全员列阵、标题和掌声一路到暗转进本篇的整段 Opening。

**CURRENT QUALIFICATION**

Opening 结构和选择理由已锁定；投稿第一句中一个名词仍不稳定，页面不发布该句逐字。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: Opening 的完整结构和选择理由可直接面向读者，投稿首句不影响理解。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-028 · yt-034011-blood-flick-research

- Track / Time: YT ≈03:40:11
- Status: `qualified`
- Flags: `has-internal-qualification`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-034011-blood-flick-research.json`

**CURRENT TITLE**

汐谷的北辰一刀流与血振り研究

**CURRENT SUMMARY**

汐谷查流派后提出血振り，并在两位演员之间共享研究；制作讨论最后把这一动作留在正式演出中。

**CURRENT QUALIFICATION**

研究发案与最终采用已锁；共享接收者不在页面强锁为单一人名。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把制作锁定替换为制作讨论最后保留，避免把编辑流程词带给读者。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 汐谷查流派后提出血振り，并在两位演员之间共享研究；制作讨论最后把这一动作留在正式演出中。
- Reader note: —

### RCOPY-029 · yt-034507-space-close-departure

- Track / Time: YT ≈03:45:07
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-034507-space-close-departure.json`

**CURRENT TITLE**

名场面复盘结束，小松再次离席处理 Space

**CURRENT SUMMARY**

《俺知》名场面环节结束后，小松离开主直播去收尾 X Space；主区暂时继续进行。

**CURRENT QUALIFICATION**

离席点按 canonical 约 03:45:07 定位；不为 YT 与 Space 建立固定 offset。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把短暂分流换成主区继续进行的自然描述，时间锚点仍留在内部资格说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 《俺知》名场面环节结束后，小松离开主直播去收尾 X Space；主区暂时继续进行。
- Reader note: —

### RCOPY-030 · yt-035300-space-close-return

- Track / Time: YT ≈03:53:00
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-035300-space-close-return.json`

**CURRENT TITLE**

Space 收束后，小松回到 Bingo 前的主直播

**CURRENT SUMMARY**

第二段 Space 已告一段落，小松返回主区；节目从跨平台对话切换到集中返还生日礼物的 Bingo。

**CURRENT QUALIFICATION**

返回点按 canonical 约 03:53:00 定位。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有文案清楚说明第二段 Space 收束、回到主区及 Bingo 的承接。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-031 · yt-035520-bingo-rules-patched

- Track / Time: YT ≈03:55:20
- Status: `verified`
- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/events/yt-035520-bingo-rules-patched.json`

**CURRENT TITLE**

Bingo 规则越玩越多，口令也临时改掉

**CURRENT SUMMARY**

一开始每人一张卡，同一时刻多人 Bingo 的单份奖品会流局；玩到后面又临时加入“double Bingo 优先”。连中奖口令也从普通“Bingo”改成了“俺を知ってくれ”。

**CURRENT QUALIFICATION**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把规则 patch 改成现场规则逐步增加的读者叙事，不改变 Bingo 规则事实。

**APPROVED REPLACEMENT**

- New title: Bingo 规则越玩越多，口令也临时改掉
- New summary: 一开始每人一张卡，同一时刻多人 Bingo 的单份奖品会流局；玩到后面又临时加入“double Bingo 优先”。连中奖口令也从普通“Bingo”改成了“俺を知ってくれ”。
- Reader note: —

### RCOPY-032 · yt-040000-tumbler-mitsutomi

- Track / Time: YT ≈04:00:00
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-040000-tumbler-mitsutomi.json`

**CURRENT TITLE**

新選組“誠”杯落到光富手中

**CURRENT SUMMARY**

第二轮 64 号由光富崇雄中奖，奖品是新選組／瀬戸焼き“誠”杯类物件。

**CURRENT QUALIFICATION**

中奖者与杯类物件已由多源锁定；品牌、材质和正式 SKU 仍需画面复核，因此不在页面扩写。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 早先误听属于内部纠错历史；页面只保留中奖者和当前可确认的奖品品类。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 第二轮 64 号由光富崇雄中奖，奖品是新選組／瀬戸焼き“誠”杯类物件。
- Reader note: —

### RCOPY-033 · yt-040405-amazon-hama

- Track / Time: YT ≈04:05:05
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-040405-amazon-hama.json`

**CURRENT TITLE**

Amazon 5000 円×2：寺島与堀金同时 Bingo

**CURRENT SUMMARY**

23 番一出，两个人同时 Bingo，正好撞上两份 Amazon 5000 円礼券。

**CURRENT QUALIFICATION**

寺島可由现场反应直接确认。堀金并非当秒清晰点名，而是由佐藤已中奖退出、濱与矢野最终只领取参加赏，以及完整中奖者集合排除锁定；采用 B+/A-。semantic override：docs/komatsu36_semantic_patch_20260810.md 第 1～3 节。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 2026-08-10 semantic override：读者层只保留寺島与堀金同时 Bingo 的事件事实；堀金的排除链保留在内部 qualification，濱未中主奖，抢卡另拆 Event。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 23 番一出，两个人同时 Bingo，正好撞上两份 Amazon 5000 円礼券。
- Reader note: —

### RCOPY-034 · yt-040539-fan-inoue

- Track / Time: YT 04:05:39
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-040539-fan-inoue.json`

**CURRENT TITLE**

Amazon 礼券后，“誠”扇子由井上中奖

**CURRENT SUMMARY**

39 号让井上雄貴拿到新選組“誠”扇子；紧接高额 Amazon 礼券后的现场温差，又被吐槽得十分现实。

**CURRENT QUALIFICATION**

扇子品类、39 号与中奖者已锁定；品牌、材质和正式商品名仍属于画面复核项。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 扇子、号码和中奖者已构成完整事件，商品细节限制留在内部资格说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-035 · yt-041453-kano-tbolan

- Track / Time: YT ≈04:14:53
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-041453-kano-tbolan.json`

**CURRENT TITLE**

狩野卡拉 OK 点了 T-BOLAN

**CURRENT SUMMARY**

前段おじさん化最终在二次会歌单中落到 T-BOLAN；同一时段小松在近场读取 Super Chat，桌边成员持续插话。

**CURRENT QUALIFICATION**

歌曲时间为当前粉丝时间轴与复核稿共同定位；并发标注只表达同段重叠，不主张逐句对齐。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有摘要能同时交代 T-BOLAN 与 Super Chat 的现场并行，不需要逐句对齐。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-036 · yt-041810-snack-akemi

- Track / Time: YT ≈04:18:10
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-041810-snack-akemi.json`

**CURRENT TITLE**

歌声间隙冒出的北九州 Snack Akemi 轶事

**CURRENT SUMMARY**

狩野唱歌时，桌边话题短暂拐到北九州和 Snack Akemi，卡拉 OK 旁边又开出一段闲聊。

**CURRENT QUALIFICATION**

复核稿确认轶事本身；起止点为事件级定位，不细分重叠发言者。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: Snack Akemi 轶事作为卡拉 OK 旁支已经清楚，事件级时间限制不需读者说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-037 · yt-042117-keserasera

- Track / Time: YT ≈04:21:17
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-042117-keserasera.json`

**CURRENT TITLE**

滨以《ケセラセラ》接棒

**CURRENT SUMMARY**

滨健人接过卡拉 OK 主唱；小松的 Super Chat 读取与桌边合いの手仍在同一时间段并行。

**CURRENT QUALIFICATION**

曲目与事件区间由时间轴、复核稿共同确认；不刊载歌词，也不把并发标注解释为逐句同步。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有摘要自然说明滨接棒和现场并行话题，不发布歌词。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-038 · yt-042252-seigura-superchat

- Track / Time: YT ≈04:22:52
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-042252-seigura-superchat.json`

**CURRENT TITLE**

声优 Grand Prix 的 Super Chat 引发现地反应

**CURRENT SUMMARY**

小松读到来自声优 Grand Prix 一侧的 Super Chat，现场立即作出反应。

**CURRENT QUALIFICATION**

只保留可复核的发送方称呼与现场反应；账号显示名细节、金额均不作断言。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 发送方称呼与现场反应足以支撑事件，账号细节和金额不影响理解。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-039 · yt-042314-hama-self-intro

- Track / Time: YT ≈04:23:14
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-042314-hama-self-intro.json`

**CURRENT TITLE**

滨在歌曲间隙插入自我评价

**CURRENT SUMMARY**

滨利用歌曲间隙，从 Sigma Seven 所属年数谈到时尚、扑克、演技、音乐、将棋与职业年限，并用等级式评价制造笑点。

**CURRENT QUALIFICATION**

话题次序可确认；音乐与职业年限等个别等级听辨保留不确定性，不作为精确逐项成绩表刊载。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 等级式自评的喜剧结构已清楚，未把不稳定细节写成成绩表。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-040 · yt-043520-ojamajo

- Track / Time: YT ≈04:35:20
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-043520-ojamajo.json`

**CURRENT TITLE**

多人合流《おジャ魔女カーニバル!!》

**CURRENT SUMMARY**

狩野、滨、寺岛与井上等人加入多人卡拉 OK，散场后的二次会进一步扩大。

**CURRENT QUALIFICATION**

参与者与曲目按复核稿确认；不拆分重叠说话人，也不刊载歌词。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 多人合流和二次会扩张已完整，重叠发言与歌词限制不必展示。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-041 · yt-044023-wheel-song

- Track / Time: YT ≈04:40:23
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-044023-wheel-song.json`

**CURRENT TITLE**

井上与汐谷组成 Heisuke 组合演唱《Wheel Song》

**CURRENT SUMMARY**

井上雄貴和汐谷文康以 Heisuke 组合继续唱卡拉 OK。第一次“结束”以后，节目反而又多出一段二次会。

**CURRENT QUALIFICATION**

组合、曲目与事件位置由复核稿确认；不刊载歌词。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: Heisuke 组合、曲目和加演段落已经可读。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-042 · yt-044513-fantastipo

- Track / Time: YT ≈04:45:13
- Status: `qualified`
- Flags: `has-internal-qualification`, `technical-language`
- Source pointer: `src/content/projects/komatsu36/events/yt-044513-fantastipo.json`

**CURRENT TITLE**

滨与狩野合唱《ファンタスティポ》

**CURRENT SUMMARY**

滨与狩野合唱时，小松在另一条近场线上加速处理尚未读完的 Super Chat。

**CURRENT QUALIFICATION**

曲目、演唱者与并发关系可确认；并发标注不代表逐句同步，歌词不刊载。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 合唱与 Super Chat 并行是事件重点，内部并发标注不向读者暴露。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-043 · yt-045435-shuji-akira

- Track / Time: YT ≈04:54:35
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/yt-045435-shuji-akira.json`

**CURRENT TITLE**

《青春アミーゴ》之后的“修二与彰”接话

**CURRENT SUMMARY**

合唱结束后，现场继续以“修二与彰”相关台词接话，短暂延长余兴。

**CURRENT QUALIFICATION**

台词主题与出现位置可确认；具体说话人分配及现实指代对象未确认，因此不作进一步归属。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 现有摘要保留余兴延长这一作用，不对未确认的说话人和现实指代做归属。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-044 · sp2-000311-account-hijack

- Track / Time: SP2 00:03:11
- Status: `qualified`
- Flags: `has-internal-qualification`
- Source pointer: `src/content/projects/komatsu36/events/sp2-000311-account-hijack.json`

**CURRENT TITLE**

账号上的发言者自称“乗っ取り”

**CURRENT SUMMARY**

这段发言只与室账号有关；对方自称“乗っ取り”，不能据此认作室元気本人。

**CURRENT QUALIFICATION**

账号归属与实际发言者不得合并。

**RECORDED DECISION**

- Copy action: `rewrite`
- Reader note action: `none`
- Editorial note: 把页面、原始区分等编辑语句改成直接的身份边界说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: 这段发言只与室账号有关；对方自称“乗っ取り”，不能据此认作室元気本人。
- Reader note: —

### RCOPY-045 · sp2-025344-uchida-forgotten

- Track / Time: SP2 ≈02:53:44
- Status: `qualified`
- Flags: `has-internal-qualification`, `evidence-language`, `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/sp2-025344-uchida-forgotten.json`

**CURRENT TITLE**

最后才想起漏问内田生日

**CURRENT SUMMARY**

清典报完生日后，小松突然意识到刚才和内田聊了很久，却忘了问他的生日。

**CURRENT QUALIFICATION**

人名采用双高质量 ASR 与人工回听当前首选；逐字 A 级仍可复核短窗。

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 事件摘要已清楚表达漏问生日的笑点，逐字听辨限制留在内部资格说明。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

### RCOPY-046 · sp2-025404-public-offer

- Track / Time: SP2 02:54:04
- Status: `verified`
- Flags: `editorial-meta-language`
- Source pointer: `src/content/projects/komatsu36/events/sp2-025404-public-offer.json`

**CURRENT TITLE**

续报还没公布，清典先收到“公开 offer”

**CURRENT SUMMARY**

Space 收尾时，小松预告 YouTube 稍后会发布《俺を知ってくれ！》续报，清典当场表示自己还没听说；小松随即让他把那天行程空出来，现场自己吐槽成“公开 offer”。

**CURRENT QUALIFICATION**

—

**RECORDED DECISION**

- Copy action: `keep`
- Reader note action: `none`
- Editorial note: 这里的“发布”是小松预告稍后公开续报的现场动作，不是档案编辑流程词；现有摘要保留公开邀约与未正式发表出演的边界。

**APPROVED REPLACEMENT**

- New title: —
- New summary: —
- Reader note: —

## Thread candidates

### RCOPY-047 · bingo-payback

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/bingo-payback.md`

**CURRENT DECK**

小松打算用Bingo形式一次还清大家的生日礼物，有人中奖，有人扑空，有人空手而来却+1000円。

**CURRENT BODY**

小松本来只是想用 Bingo 一次还清大家的生日礼物，结果规则一路边玩边补。有人很快拿到主奖，有人始终扑空；濱明明空手来，Amazon 卡没抢到，最后却和矢野一起多拿了 1000 円参加赏。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 2026-08-10 semantic override：移除濱中奖判断，并把抢寺島卡作为独立节点接入。

**APPROVED REPLACEMENT**

- New deck: 小松打算用Bingo形式一次还清大家的生日礼物，有人中奖，有人扑空，有人空手而来却+1000円。
- New body: 小松本来只是想用 Bingo 一次还清大家的生日礼物，结果规则一路边玩边补。有人很快拿到主奖，有人始终扑空；濱明明空手来，Amazon 卡没抢到，最后却和矢野一起多拿了 1000 円参加赏。

### RCOPY-048 · birthday-payback

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/birthday-payback.md`

**CURRENT DECK**

小松先和観世约好“以后生日也要祝回来”，问到清典时却突然想起：前面和内田聊了那么久，自己根本没问人家生日。

**CURRENT BODY**

小松先对観世临时立下“以后也要记得给别人庆生”的规则，到了清典这里又认真问起 11 月 14 日。结果话刚说完，他突然想起：自己前面明明和内田聊了很久，却完全忘了问内田生日。规则是自己立的，先被绊住的也是自己。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 历史 RAW 恢复后，直接讲清从立规则到忘问内田生日的回收，不再向读者展示旧听辨谨慎说明。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 小松先对観世临时立下“以后也要记得给别人庆生”的规则，到了清典这里又认真问起 11 月 14 日。结果话刚说完，他突然想起：自己前面明明和内田聊了很久，却完全忘了问内田生日。规则是自己立的，先被绊住的也是自己。

### RCOPY-049 · broken-sword

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/broken-sword.md`

**CURRENT DECK**

濱的刀真的撞断成两半了，矢野那把只被削伤；下一句又正好是“用刀互砍的时代结束了”，自然到不少观众以为本来就是演出。

**CURRENT BODY**

大家复盘的是昼场那次真事故：濱的刀真的折断，矢野那把只被削伤，碎片飞向舞台袖，幸好没有人受伤。偏偏下一句就是「刀で斬り合う時代は終わりじゃ」，自然得让不少观众以为本来就是演出。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 用事故经过和下一句台词构成故事，不再解释档案记录坐标。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 大家复盘的是昼场那次真事故：濱的刀真的折断，矢野那把只被削伤，碎片飞向舞台袖，幸好没有人受伤。偏偏下一句就是「刀で斬り合う時代は終わりじゃ」，自然得让不少观众以为本来就是演出。

### RCOPY-050 · hama-paid-drinking

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/hama-paid-drinking.md`

**CURRENT DECK**

没带礼物却一路吃喝到最后；Amazon 轮自己没中，还伸手去抢寺島的卡，最后只拿到 1000 円参加赏，于是现场真的变成了“ギャラ飲み”。

**CURRENT BODY**

濱这晚从一句「俺ない」开始就没停过：没带礼物、喝高价酒、只关心自己出演作品，到了 Bingo 又盯上 Amazon 礼券，自己没中还伸手去抢寺島的卡。最后他和矢野作为仅剩的两名未中奖者拿到 1000 円参加赏，濱干脆自己把它说成「ギャラ＋1000円」。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 2026-08-10 semantic override：濱没有中 Amazon 主奖，人物线终点改为抢卡与 1000 円参加赏。

**APPROVED REPLACEMENT**

- New deck: 没带礼物却一路吃喝到最后；Amazon 轮自己没中，还伸手去抢寺島的卡，最后只拿到 1000 円参加赏，于是现场真的变成了“ギャラ飲み”。
- New body: 濱这晚从一句「俺ない」开始就没停过：没带礼物、喝高价酒、只关心自己出演作品，到了 Bingo 又盯上 Amazon 礼券，自己没中还伸手去抢寺島的卡。最后他和矢野作为仅剩的两名未中奖者拿到 1000 円参加赏，濱干脆自己把它说成「ギャラ＋1000円」。

### RCOPY-051 · kano-ojisan

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/kano-ojisan.md`

**CURRENT DECK**

dad joke、厕所离席和一路累积的おじさん吐槽，到了卡拉 OK 终于落在 T-BOLAN 的选曲上。

**CURRENT BODY**

狩野前面一路被吐槽越来越“おじさん”，厕所离席只是其中一个小插曲。等二次会真的点到 T-BOLAN，这条一路累积的吐槽终于直接落在歌单上。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 直接交代连续笑点如何落到歌单，不再向读者说明资料缺口。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 狩野前面一路被吐槽越来越“おじさん”，厕所离席只是其中一个小插曲。等二次会真的点到 T-BOLAN，这条一路累积的吐槽终于直接落在歌单上。

### RCOPY-052 · muro-account

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/muro-account.md`

**CURRENT DECK**

第一段结束前只来得及看到账号；重开以后终于有人开口，却又自称“乗っ取り”。

**CURRENT BODY**

第一段 Space 结束前只来得及看到室元気的账号。重开以后，账号再次出现，对方却自己说出了「乗っ取り」。所以这里能确认的是“室元気账号参与了 Space”，而不是“室元気本人终于来了”。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 保留必须公开的身份边界，同时把它写成账号插曲本身的故事。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 第一段 Space 结束前只来得及看到室元気的账号。重开以后，账号再次出现，对方却自己说出了「乗っ取り」。所以这里能确认的是“室元気账号参与了 Space”，而不是“室元気本人终于来了”。

### RCOPY-053 · ore-shiri-making-of

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/ore-shiri-making-of.md`

**CURRENT DECK**

无论是聚焦在舞台专业表现力，还是选择从轻松的俳句即兴入手都是好选择。动作指导清典先生的投稿详细评点并总结了大家的努力。

**CURRENT BODY**

大家挑出来的“名场面”根本不是同一种东西：有人认真聊武器、流派和身体重心，有人记住的是方言、朗读节奏，甚至还有俳句即兴。清典先生的长投稿又从动作指导的角度补上训练和制作细节，整段复盘也因此不只是“选一个最喜欢的场面”。

**CURRENT TRANSITIONS**

- 主直播的制作复盘在 SP2 得到动作监督的专业总结——

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 改成制作过程的故事总结，不再解释 Thread 的编辑分层。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 大家挑出来的“名场面”根本不是同一种东西：有人认真聊武器、流派和身体重心，有人记住的是方言、朗读节奏，甚至还有俳句即兴。清典先生的长投稿又从动作指导的角度补上训练和制作细节，整段复盘也因此不只是“选一个最喜欢的场面”。

### RCOPY-054 · russian-takoyaki

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/russian-takoyaki.md`

**CURRENT DECK**

狩野随口提了一句俄罗斯章鱼烧游戏。芥末那颗很快被识破，却又被大家放在一边，约 80 分钟后才由完全不知前情的堀金一口踩中。

**CURRENT BODY**

濱最早抽中那颗章鱼烧，看见已经绿到几乎能确认是 wasabi，因为「ひよって」又直接放了回去。大家很快也识破了那颗芥末章鱼烧，紧接着却忙着看赛马和跑后面的节目流程，再没人管它。赛马是否真的导致“忘记”并不能确认，只能说时间上正好挨在一起。约 80 分钟后，完全不知道前情的堀金蒼平一口踩雷，现场直接冒出一句「時限爆弾すぎるだろ！」。

**CURRENT TRANSITIONS**

- 赛马开跑，现场注意力被直接切走。约 80 分钟后——

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 把因果边界写成自然叙述，移除现场原声和不把它写成等编辑判断句。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 濱最早抽中那颗章鱼烧，看见已经绿到几乎能确认是 wasabi，因为「ひよって」又直接放了回去。大家很快也识破了那颗芥末章鱼烧，紧接着却忙着看赛马和跑后面的节目流程，再没人管它。赛马是否真的导致“忘记”并不能确认，只能说时间上正好挨在一起。约 80 分钟后，完全不知道前情的堀金蒼平一口踩雷，现场直接冒出一句「時限爆弾すぎるだろ！」。

### RCOPY-055 · shugo-yakiniku

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/shugo-yakiniku.md`

**CURRENT DECK**

奇怪的人带着奇怪的话突然闯进 Space，说完“今晚吃烧肉”就消失了。重开Space后的小松只好替这句简短留言补成庆生。

**CURRENT BODY**

仲村好不容易接进来，几乎只留下“今晚去吃烤肉”就又掉线。重开以后，小松只好自顾自替他补成“因为我生日很开心，所以去吃烤肉庆祝”，把一条信息极少的留言硬凑成完整祝福。

**CURRENT TRANSITIONS**

- 主直播随后把这段简短留言复述给新到场的朋友——

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 用掉线与小松强行补祝福构成故事，不再评论资料信息量。

**APPROVED REPLACEMENT**

- New deck: 奇怪的人带着奇怪的话突然闯进 Space，说完“今晚吃烧肉”就消失了。重开Space后的小松只好替这句简短留言补成庆生。
- New body: 仲村好不容易接进来，几乎只留下“今晚去吃烤肉”就又掉线。重开以后，小松只好自顾自替他补成“因为我生日很开心，所以去吃烤肉庆祝”，把一条信息极少的留言硬凑成完整祝福。

### RCOPY-056 · terashima-big-dream

- Flags: `resolved-decision`
- Source pointer: `src/content/projects/komatsu36/threads/terashima-big-dream.md`

**CURRENT DECK**

小松在寺岛递来的三张手写券中选了其二：陪寿星折腾杀阵，外加Big Dream赛马下注。滨想抢券？“那就给你34円！”最后全员围在20:50赛马直播前，只等一句——惜しい！

**CURRENT BODY**

小松没有选最安全的 36J，而是拿走了“陪寿星折腾杀阵”和 Big Dream 赛马下注。寺島另外买的是 5-6，小松拿到的生日礼物票则是 5-8 wide。全员最后围在 20:50 的赛马直播前喊出「惜しい！」，再认真一算——其实什么都没中。

**CURRENT TRANSITIONS**

—

**RECORDED DECISION**

- Copy action: `rewrite`
- Editorial note: 用两张票和最终归零讲清笑点，不再写事件线如何裁决。

**APPROVED REPLACEMENT**

- New deck: —
- New body: 小松没有选最安全的 36J，而是拿走了“陪寿星折腾杀阵”和 Big Dream 赛马下注。寺島另外买的是 5-6，小松拿到的生日礼物票则是 5-8 wide。全员最后围在 20:50 的赛马直播前喊出「惜しい！」，再认真一算——其实什么都没中。
