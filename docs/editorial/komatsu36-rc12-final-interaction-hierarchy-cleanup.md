...
> 日期：2026-08-10
> 基线：`fd7812b feat(komatsu36): compact mobile timeline`
> 目标：在不重开 RC12 大模块的情况下收口两个 P0 交互／信息层级问题
...
## 4. 下一批 P1 待办（本轮只记录，不实施）

### P1-C 中文 UI label token

新增 `.archive-label-zh`，只迁移中文 taxonomy／动作标签：`当前播放`、`切换来源`、`选择时间线来源`、`跨平台`、`Space 来宾`、`账号出现`、`LINE 电话`、`相关上下文`。时间、代码、平台短码和 Event count 继续 mono；正文与 Event title 继续 serif。不得全局改 `.archive-kicker`。

### P1-D Media Sources disclosure

Hero 下常驻摘要：`YouTube 主直播 + X Space ① / ②`、`三路媒体 · 各自原视频时间`、`查看 3 个媒体来源、时长与来源证明 ▾`。完整三卡放入原生 `<details>`／`<summary>`，保留 keyboard、无脚本和现有 source selector／provenance／browse 合同。首批不退休 Source Event Index；先做视觉降级，再单独评估 `浏览事件 →` 是否改为 `在 Timeline 浏览 →`。
